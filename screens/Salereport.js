...
export default Salereport = ({navigation}) => {
  ...
  const getData = async () => {
    ...
    if (!loading && !isListEnd) {
      ...
      fetch('https://customerapp.toniapharmacy.com.ng/dev/api/authentication/get_salereport?user_id='+user_id+'&&offset='+offset)
        //Sending the currect offset with get request
        .then((response) => response.json())
        .then((responseJson) => {
          //Successful response from the API Call
          console.log(responseJson);
          if (responseJson.salereport.length > 0) {
            setOffset(offset + 1);
            //After the response increasing the offset for the next API call.
            setDataSource([...dataSource, ...responseJson.salereport]);
            setLoading(false);
          } else {
            setIsListEnd(true);
            setLoading(false);
